...
public class Rijndael {
    ...
    public static Rijndael getInstance(byte[] key, AESMode mode) {
        ...
        final String cacheKey = StringUtils.join(new Object[]{hexKey,
                StringUtils.replace(mode.getName(), "/", "_")}, "_");
        if (AES_CACHES.containsKey(cacheKey)) {
            return AES_CACHES.get(cacheKey);
        }
        Rijndael o = new Rijndael(key, mode);
        AES_CACHES.put(cacheKey, o);
        return o;
    }

    /**
     * 根据给定的私密密钥、IV向量和默认的 {@code AES/CBC/PKCS5Padding} 填充方式，获得一个
     * {@code Rijndael}。
     *
     * @param key 私密密钥。
     * @param iv  IV 向量。
     * @return 给定的私密密钥、IV向量和默认的 {@code AES/CBC/PKCS5Padding} 填充方式的
     * {@code Rijndael}。
     */
    ...
    public static Rijndael getInstance(byte[] key, byte[] iv, AESMode mode) {
        ...
        final String cacheKey = StringUtils.join(new Object[]{hexKey, hexIV,
                StringUtils.replace(mode.getName(), "/", "_")}, "_");
        if (AES_CACHES.containsKey(cacheKey)) {
            return AES_CACHES.get(cacheKey);
        }
        Rijndael o = new Rijndael(key, iv, mode);
        AES_CACHES.put(cacheKey, o);
        return o;
    }
    ...
    /**
     * 根据给定的私密密钥、IV向量和默认的 {@code AES/CBC/PKCS5Padding} 填充方式，构造一个新的
     * {@code Rijndael}。
     *
     * @param key 私密密钥。
     * @param iv  IV 向量。
    ...
    private void validateKey(byte[] key) {
        if (key == null || key.length < KEY_LENGTH) {
            throw new IllegalArgumentException(String.format(
                    "Invalid AES key length: %s bytes", key == null ? 0
                            : key.length));
        }
    }
    ...
    private void validateIV(byte[] iv) {
        if (iv == null || iv.length < IV_LENGTH) {
            throw new IllegalArgumentException(String.format(
                    "Wrong IV length: must be %s bytes long", IV_LENGTH));
        }
    }
     ...
     * <p/>
     * 处理 data 缓冲区中从 {@code offset} 开始（包含）的前 {@code length} 个字节以及可能在上一次
     * {@code update} 操作过程中已缓存的任何输入字节，其中应用了填充（如果需要）。结果存储在新缓冲区中。
     * <p/>
     * 结束时，此方法将把此类中的 {@code cipher} 加密对象重置为上一次调用 {@code init}
     * 初始化得到的状态。即重置该对象，可供加密或解密（取决于调用 init 时指定的操作模式）更多的数据。
     *
     ...
     * 个字节以及可能在上一次 {@code update} 操作过程中已缓存的任何输入字节，其中应用了填充（如果需要）。结果存储在
     * {@code outputData} 缓冲区中从 {@code outputOffset}（包含）开始的位置。
     * <p/>
     ...
     */
    public int encrypt(byte[] inputData, int inputOffset, int inputLen,
                       byte[] outputData, int outputOffset) {
        try {
            return enc.doFinal(inputData, inputOffset, inputLen, outputData,
                    outputOffset);
        } catch (Exception ex) {
            writeEncryptLogger(ex);
        }
     ...
     * @return {@code outputData} 中存储的字节数。
     */
    public int decrypt(byte[] inputData, int inputOffset, int inputLen,
                       byte[] outputData, int outputOffset) {
        try {
            return dec.doFinal(inputData, inputOffset, inputLen, outputData,
                    outputOffset);
        } catch (Exception ex) {
            writeDecryptLogger(ex);
        }
     ...
     * {@code encoding == null || "".equals(encoding)}，则字符编码默认为
     * {@code ISO-889-1}
     * <p/>
     ...
     * <p/>
     * 如果 {@code encoding == null || "".equals(encoding)}，则字符编码为
     * {@code ISO-8859-1}。
     *
     * @param input    要加密的字符串。
     * @param encoding 字符编码。
    ...
    /**
     * 将给定的 {@code base64} 编码格式字符串还原成 {@code byte} 数组， 然后对该数据进行 {@code AES}
     * 解密。如果解密失败，则返回空的{@code byte}数组。
     * <p/>
     ...
     * 使用指定的字符编码加密给定的字符串。
     * <p/>
     * 该方法先将明文字符串根据指定字符编码转换成 {@code byte[]}，然后调用
     * {@link #encryptToBase64(byte[])} 进行数据加密。 如果
     * {@code encoding == null || "".equals(encoding)}，则字符编码默认为
     * {@code ISO-8859-1}。
     *
     * @param input    要加密的字符串。
    ...
    /**
     * 将给定的 {@code base64} 编码格式字符串还原成 {@code byte} 数组， 然后对该数据进行 {@code AES}
     * 解密，返回明文字符串。如果解密失败，则返回 {@code null}。
     * <p/>
    ...
    private void writeEncryptLogger(Throwable cause) {
        LOGGER.error(String.format("AES decrypt failed. Cause: %s",
                cause.getMessage()));
    }
    ...
    private void initEncryptionCipher() {
        if (enc == null) {
            try {
                ...
                LOGGER.error(String
                        .format("The current platform no such algorithm - %s, Cause: %s",
                                ALGORITHM, ex.getMessage()));
            } catch (NoSuchPaddingException ex) {
                LOGGER.error(String.format(
                        "The current platform no such padding - %s, Cause: %s",
                        mode.getName(), ex.getMessage()));
            } catch (InvalidKeyException ex) {
                LOGGER.error(StringUtils.join(new Object[]{
                        "Invalid key - ",
                        (keySpec != null ? keySpec.toString() : "encrypt key. "),
                        " Cause: ", ex.getMessage()}));
            } catch (InvalidAlgorithmParameterException ex) {
                LOGGER.error(StringUtils.join(new Object[]{
                        "Invalid algorithm parameter - ",
                        (ivSpec != null ? ivSpec.toString()
                                : "encrypt iv parameter. "), "Cause: ",
                        ex.getMessage()}));
            }
        }
    }
    ...
    private void initDecryptionCipher() {
        if (dec == null) {
            try {
                ...
                LOGGER.error(StringUtils.join(new Object[]{
                        "Invalid key - ",
                        (keySpec != null ? keySpec.toString() : "decrypt key. "),
                        " Cause: ", ex.getMessage()}));
            } catch (InvalidAlgorithmParameterException ex) {
                LOGGER.error(StringUtils.join(new Object[]{
                        "Invalid algorithm parameter - ",
                        (ivSpec != null ? ivSpec.toString()
                                : "decrypt iv parameter. "), "Cause: ",
                        ex.getMessage()}));
            }
        }
    }
...
//		System.setProperty("rootDir", "E:\\WorkSpace\\Idea\\Java\\GameSocketServer");
        ...
        Rijndael ecb_aes = new Rijndael(key);
        ...
        System.out.println("");
        Rijndael cbc_aes = new Rijndael(key, iv, AESMode.CBC_NO);
        ...
        Rijndael cbc_aes_pkcs5 = new Rijndael(key, iv, AESMode.CBC_PKCS5);
...
//        System.out.println(cbc_aes_pkcs5.encrypt(bytes).length);
        ...
        String str = cbc_aes_pkcs5.encryptToHex(bytes);
        System.out.println(String.format("密文：%s", str));

        bytes = cbc_aes_pkcs5.decryptHex(str);
        GameInput input = new GameInput(bytes);
        long id = input.readLong();
        String name = input.readUTF();
        System.out.println(String.format("解密：%s%s", id, name));
